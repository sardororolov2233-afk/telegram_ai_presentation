...
def generate_print_html(slides: list[SlideData], topic: str, style: str = "professional") -> str:
    """PDF ga chiqarish uchun mo'ljallangan soddalashtirilgan HTML page."""
    theme = THEMES.get(style, THEMES["professional"])
    is_dark = style != "minimal"
...
<style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  /* Title slide */
  .title-slide {{ align-items:center; text-align:center; }}
  .main-title {{ font-size: 80px; font-weight:700; line-height:1.2; margin-bottom:30px; }}
  .title-line {{ width:120px; height:6px; border-radius:3px; margin:0 auto 40px; }}
  .subtitle {{ font-size: 36px; opacity:.75; }}
  .slide-number {{ position:absolute; top:40px; right:50px; font-size:24px; opacity:.3; letter-spacing:3px; }}

  /* Section slide */
  .section-slide {{ align-items:center; justify-content:center; text-align:center; }}
  .section-number {{ font-size: 150px; font-weight:900; opacity:.15; position:absolute; top:40px; left:60px; }}
  .section-title {{ font-size: 70px; font-weight:700; z-index:1; }}

  /* Content slide */
  .content-slide {{ padding:0; }}
  .slide-accent {{ position:absolute; left:0; top:0; bottom:0; width:12px; }}
  .slide-inner {{ padding: 60px 80px; display:flex; flex-direction:column; height:100%; }}
  .slide-header {{ display:flex; align-items:center; gap:24px; margin-bottom:40px; }}
  .slide-num {{ font-size: 30px; font-weight:700; min-width:40px; }}
  .slide-title {{ font-size: 56px; font-weight:700; line-height:1.3; }}
  .content-card {{ flex:1; border-radius:20px; padding:40px 50px; }}

  /* Conclusion */
  .conclusion-slide {{ align-items:center; text-align:center; }}
  .conclusion-icon {{ font-size: 90px; margin-bottom:30px; opacity:.6; }}
  .conclusion-title {{ font-size: 64px; font-weight:700; margin-bottom:40px; }}
  .conclusion-bullets {{ text-align:left; max-width:1000px; }}

  /* Bullets */
  .bullet-list {{ list-style:none; display:flex; flex-direction:column; gap:20px; }}
  .bullet {{ font-size: 32px; line-height:1.5; padding-left:30px; position:relative; opacity:.9; }}
  .bullet::before {{ content:"▸"; position:absolute; left:0; color:{theme['accent']}; font-size: 36px; }}
</style>
</head>
<body>
  {slides_html}
